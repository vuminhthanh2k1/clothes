...
module.exports = function (Cart) {
    Cart.addToCart = async function (ctx) {
    ...
    Cart.afterRemote("find", async (ctx) => {
        try {
            const pg = (0, knex_1.knex)("cart").count().toString();
            const data = await (0, native_sql_1.executeNativeSql)(Cart.app.dataSources.postgres.connector, pg, []);
    ...
    Cart.statisticByAmount = async function (ctx) {
    ...
    Cart.statisticByRevenue = async function (ctx) {
        const year = ctx.req.query.year;
        let dayOfFeburary = 0;
        if (year % 4 == 0) {
            dayOfFeburary = 29;
    ...
    Cart.changeAmount = async function (ctx) {
